...
for(let i=1;i<=5;i++){const s=Math.round(mx*i/5);const k=ks.length?ks.reduce((p,q)=>Math.abs(q-s)<Math.abs(p-s)?q:p):s;c.append(el('line',{x1:X(s),x2:X(s),y1:H-B,y2:H-B+4,stroke:'#666'}));const t=el('text',{x:X(s),y:H-6,fill:'#888','font-size':11,'text-anchor':'end'});const hrs=e=>{const m=/(\d+)h(\d+)m|(\d+)m(\d+)s/.exec(e||'');return m?(m[1]?+m[1]+m[2]/60:m[3]/60).toFixed(1)+' h':''};t.textContent=(s>=1000?Math.round(s/1000)+'k':s)+(ks.length?' · '+hrs(EL[k]):'');c.append(t)}
const ax=el('text',{x:L,y:H-6,fill:'#666','font-size':11});ax.textContent='steps · wall h';c.append(ax);
...
steps.forEach((s,i)=>{const rs=d.filter(r=>r.step==s);const hd=`round ${N-i} of ${N} &middot; step ${s}${rs[0].elapsed?' &middot; '+rs[0].elapsed+' into the run':''}`;h+=`<details${i<OPEN?' open':''} style="border:1px solid #333;border-radius:6px;padding:.5em;margin:.6em 0;color:#aaa"><summary>${hd}</summary><table>`;
for(const r of rs) h+=`<tr><td style="color:${COL[r.name]||'#aaa'};white-space:nowrap;width:9em"><b>${r.name}</b><br><span style="color:${wc(r.wer)}">wer ${r.wer.toFixed(2)}</span></td><td style="width:6em">${btn(`/wav?run=${encodeURIComponent(view.value)}&f=${r.name}_step_${String(s).padStart(6,'0')}.wav`)}</td><td style="color:#999">input: <span style="color:#ddd">${(S[r.name]||{}).text||''}</span><br>whisper: <span style="color:#ddd">${r.heard}</span></td></tr>`;h+='</table></details>'});
