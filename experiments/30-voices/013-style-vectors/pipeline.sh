...
set -e
cd "$(dirname "$0")"
while ! grep -q "all done" ../../20-scaling/012-english-235h-overnight/runs/pipeline.log 2>/dev/null; do sleep 120; done
CK=../../20-scaling/012-english-235h-overnight/runs/overnight/ttl.pt; T=$(python3 -c "import json;print(json.load(open('config.json'))['style']['mix_t'])")
