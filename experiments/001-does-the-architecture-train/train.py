...
c = load_config()
smoke, resume = "--smoke" in sys.argv, "--resume" in sys.argv
t, d, K, L = c["ttl"], c["data"], c["latent"]["compress"], c["latent"]["dim"]
prep = P(d["prep_dir"])
run = P(os.path.join("runs", "ttl_smoke" if smoke else "ttl"))
...
def sample(step):
    ...
    model.train()
    tb.add_audio("sample", torch.from_numpy(wav)[None], step, sample_rate=d["sample_rate"])
