...
echo "[run.sh] experiment $(pwd), run $R, config.json + overrides: $*"
if [ -f runs/$R/ttl.pt ]; then echo "[run.sh] runs/$R/ttl.pt exists: resuming (step, optimizer and budget clock continue; --fresh starts over)"; set -- --resume "$@"; else echo "[run.sh] new run (no runs/$R/ttl.pt)"; fi
if [ "${PREFLIGHT:-1}" = 1 ]; then
  echo "[run.sh] stage 1/2 preflight (PREFLIGHT=0 skips it)"
  ../../preflight.sh "$@" | tee runs/preflight.log || exit 1
  B=$(tail -1 runs/preflight.log | sed -n 's/^BATCH=//p'); [ -n "$B" ] && set -- "$@" --set ttl.batch=$B
fi
echo "[run.sh] stage 2/2 training: train.py --run $R $* (foreground, Ctrl-C stops; runs/$R/progress.log has the same lines; panel http://localhost:7807)"
