...
@torch.no_grad()
def sample(step):
    """Synthesize the fixed sample sentence with the current weights, log audio to TensorBoard and disk."""
    global sampler
    ...
    if sampler is None:
        ...
        # once per run: the sample text and, if it is a training sentence, the original recording
        tb.add_text("sample/text", t["sample_text"], 0)
        orig = [r for r in rows + val if r["text"] == t["sample_text"]]
        if orig:
            import soundfile as sf
            w, sr = sf.read(os.path.join(P(d["raw_dir"]), orig[0]["path"]), dtype="float32")
            tb.add_audio("sample/original", torch.from_numpy(w if w.ndim == 1 else w.mean(1))[None], 0, sample_rate=sr)
    sampler.model = model; model.eval()
    wav, _ = sampler(t["sample_text"], sampler.zref, sampler.rmask, steps=t["sample_steps"], cfg=t["sample_cfg"])
    model.train()
    tb.add_audio("sample/generated", torch.from_numpy(wav)[None], step, sample_rate=d["sample_rate"])
    os.makedirs(os.path.join(run, "samples"), exist_ok=True)
    import soundfile as sf
    path = os.path.join(run, "samples", f"step_{step:06d}.wav")
    sf.write(path, wav, d["sample_rate"])
    try:  # Whisper is the validator: transcribe the sample, log WER and text
        ...
        hyp = urllib.request.urlopen(req, timeout=120).read().decode()
        norm = lambda x: re.sub(r"[^a-z0-9' ]", " ", x.lower()).split()
        r, h = norm(t["sample_text"]), norm(hyp)
        ...
        w = dd[len(h)] / len(r)
        tb.add_scalar("sample/wer", w, step); tb.add_text("sample/whisper", f"wer={w:.2f} | {hyp}", step)
        line = f"SAMPLE step={step} wer={w:.2f} | {hyp}"
        print(line, flush=True); log.write(line + "\n"); log.flush()
    except Exception as ex:
        print(f"SAMPLE step={step} whisper failed: {ex}", flush=True)
...
while step < steps:
    ...
    if step % t["log_every"] == 0 or smoke:
        line = f"step={step} fm={lf:.4f} dp={ld:.4f} lr={lr:.2e} elapsed={time.time()-t0:.0f}s vram={torch.cuda.max_memory_allocated()/2**20 if dev=='cuda' else 0:.0f}MiB"
        print(line, flush=True); log.write(line + "\n"); log.flush()
    ...
    if step % t["val_every"] == 0 or step == steps:
        vf, vd = validate()
        line = f"VAL step={step} fm={vf:.4f} dp={vd:.4f}"
        print(line, flush=True); log.write(line + "\n"); log.flush()
