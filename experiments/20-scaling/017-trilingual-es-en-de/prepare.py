"""Mixed prep from N existing preps (data.sources: es, en, de, ...) (same AE, same latent stats; the Spanish vocab is the English vocab plus 13 chars, ids unchanged).
...
train.jsonl and val.jsonl interleave the two languages (es, en, de, es, en, de, ...) so any prefix is balanced; rows carry "lang" and an absolute "path"; text is wrapped in language tags <es>...</es> / <en>...</en> / <de>...</de> (Supertonic style) and the tags are appended to the vocab."""
...
mixed = [r for pair in zip(*(picked[l][:n] for l in picked)) for r in pair] + [r for l in picked for r in picked[l][n:]]  # es, en, de, es, en, de, ... then the remainder
