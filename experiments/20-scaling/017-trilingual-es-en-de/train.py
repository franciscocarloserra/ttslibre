...
def wer(ref, hyp):
    ...
    m = re.match(r"\s*<([a-z]{2})>", ref); lang = m.group(1) if m else t.get("lang", "en")  # numbers are spelled in the language of the tag
    norm = lambda x: re.sub(rf"[^{t['wer_chars']} ]", " ", strip(re.sub(r"</?[a-z]{2}>", "", re.sub(r"\d+", lambda m: num2words(int(m.group()), lang=lang), x.lower().replace("ß", "ss"))))).split()
