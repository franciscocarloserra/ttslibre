...
cd "$(dirname "$0")"
read T TOL SKIP < <(python3 -c "import json;g=json.load(open('config.json'))['gpu'];print(g['util_target_pct'],g['util_tolerance_pct'],g['smoke_skip_seconds'])")
for b in $(python3 -c "import json;print(*json.load(open('config.json'))['gpu']['batch_candidates'])"); do
  ...
  PID=$!; sleep $SKIP; vals=()  # skip model load / baseline sample
  ...
  echo "batch $b: gpu util ${u}% over ${#vals[@]} s (idle before: ${base}%, target ${T}±${TOL}%)  $(grep -c 'audio loss' runs/smoke_b$b/progress.log) log lines  $(tail -1 runs/smoke_b$b.out | cut -c1-80)"
done
