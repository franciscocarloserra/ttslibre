...
class DurationPredictor(nn.Module):
    ...
    def style(self, zref, rmask):  # (B, n_style, H): the duration predictor's own style tokens
        return self.ref(zref, rmask)

    def forward(self, ids, tmask, zref=None, rmask=None, style=None):
        ...
        re = self.ref_out((style if style is not None else self.style(zref, rmask)).flatten(1))
...
class TTL(nn.Module):
    ...
    def style(self, zref, rmask):
        """The voice as tensors: {"style": (B, n_style, ref_hidden) for text/vector field, "dp": (B, dp.n_style, dp.char_dim) for durations}."""
        return {"style": self.ref(zref, rmask), "dp": self.dp.style(zref, rmask)}

    def encode(self, ids, tmask, zref=None, rmask=None, style=None):
        style = style if style is not None else self.ref(zref, rmask)
